...
<html>
	...
	<body>
		...
		foreach ($rows as $row)
		{
			...
			echo '<a href="" style="' . $style . '" title="' . $row['count'] . '">' . $row['label'] . '</a> ';
		}
		echo "</p>";
		*/
		?>
		...
		*/
		...
		foreach ($rows as $row) {
			...
			foreach ($rows2 as $row2) {
				echo "<a href='#project_" . $row2['id'] . "'>" . $row2['name'] . "</a>, ";
			}
		...
		*/
		...
		foreach ($rows as $row) {
			...
			foreach ($rows2 as $row2) {
				echo "<a href='stadistics.tags.output.html#tag_" . $row2['id'] . "'>" . $row2['label'] . "</a>, ";
			}
